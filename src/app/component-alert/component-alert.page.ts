...
export class ComponentAlertPage implements OnInit {
  ...
  async abrirAlertConfirmacao(){
    ...
    let abrirAlertConfirmacao = await this.alertCtrl.create(
      {
        header : 'Confirmação',
    ...
    await abrirAlertConfirmacao.present();
  }
      ...
      buttons : [
        {
          ...
          cssClass : 'secondary',
          handler : ()=>{
            console.log('Aqui vai abrir a tela de cadastros de novos usuários')
          }
        },
        {
          text : 'Entrar',
